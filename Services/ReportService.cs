...
public class ReportService : IReportService
{
    ...
    public async Task<byte[]> ExportToPdfAsync(string reportType, Dictionary<string, string>? filter, int userId)
    {
        ...
        // Build filter label for asset-inventory
        string? filterLabel = null;
        if (reportType == "asset-inventory" && filter != null)
        {
            filter.TryGetValue("locationType", out var lt);
            filter.TryGetValue("projectId", out var pidStr);
            if (!string.IsNullOrEmpty(pidStr) && int.TryParse(pidStr, out var pidVal))
            {
                var proj = await _context.Projects.FindAsync(pidVal);
                filterLabel = proj != null ? $"Project: {proj.Name}" : $"Project ID: {pidStr}";
            }
            else if (lt == "office") filterLabel = "Location: Head Office";
            else if (lt == "site") filterLabel = "Location: All Site Assets";
        }
        ...
        if (filterLabel != null)
            logoCell.Add(new Paragraph(filterLabel).SetFont(boldFont).SetFontSize(7).SetFontColor(new DeviceRgb(50, 50, 120)).SetMarginTop(2));
